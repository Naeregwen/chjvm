...
set help.line.03=-s   : Si Mode non interactif, alors ne pas afficher de messages (sauf les erreurs).
set help.line.04=-n   : user.jvm.number = numéro de la JVM à sélectionner dans la liste. Mode non interactif.
set help.line.05=-d   : user.jvm.drive = Lettre du disque d'installation.
set help.line.06=-r   : user.jvm.root = Nom absolu du répertoire racine des JVM, sans mention du disque.
REM
set help.line.07#break=Exemple : Si le répertoire racine des JVM est C:\Outils_Dev\JAVA\
set help.line.08=Alors l'argument user.jvm.root aura pour valeur Outils_Dev\JAVA
REM
set help.line.09#break=Exemple de structure de répertoire multi JVM attendue :
set help.line.10=C:\Outils_Dev\JAVA\
set help.line.11=C:\Outils_Dev\JAVA\jdk1.6.0_45
set help.line.12=C:\Outils_Dev\JAVA\jdk1.7.0_80
set help.line.13=C:\Outils_Dev\JAVA\jdk1.8.0_144
set help.line.14=C:\Outils_Dev\JAVA\jre1.8.0_144
set help.line.15=C:\Outils_Dev\JAVA\Shell
REM
set help.line.16#break=Toutes les JVM doivent être installées dans un même répertoire de base 
set help.line.17=Et le nom de leur propre répertoire de base doit commencer par la lettre j.
set help.line.18=La lettre j est importante, parce qu'elle va être utilisée en tant que
set help.line.19=pivot du motif de recherche, pour retrouver toutes les JVM disponibles
REM
:setup
REM DEBUG
set debug=0
...
if "%r%" == "true" (
	...
	goto end.read.user.jvm.root
) else (
	if not "%r%" == "" (
		set user.jvm.root=%r%	
		if "!user.jvm.root:~0,1!" == "\" set user.jvm.root=!user.jvm.root:~1,-1!
		if "!user.jvm.root:~-1!" == "\" set user.jvm.root=!user.jvm.root:~0,-1!
		if exist %disk%\!user.jvm.root!\NUL goto found.user.jvm.root
...
if "%n%" == "true" (
	...
	goto end.read.user.jvm.number
) else ( 
	if not "%n%" == "" (
		set user.jvm.number=%n%
REM		call :is.numeric user.jvm.number
		set "not.numeric="&for /f "delims=0123456789" %%i in ("!user.jvm.number!") do set not.numeric=%%i
		if defined not.numeric goto user.jvm.number.error
		if !user.jvm.number! EQU 0 goto user.is.a.cow.moo
		call :check.for.user.jvm.number
		if defined user.jvm.number.valid goto end.read.user.jvm.number
		set user.jvm.number.invalid=1
	)
	goto end.read.user.jvm.number
)
:user.jvm.number.error
echo Erreur : l'argument qui suit -n n'est pas numérique (Entier positif ou nul non signé) 1>&2
set user.jvm.number.invalid=1
:end.read.user.jvm.number
...
:user.is.a.cow.moo
if not defined mode.silent echo Ok, on s'appelle et on se fait un cinéma
:a.cow.is.speaking.to.a.cow
if not defined mode.silent calc
goto exit
...
endlocal& set PATH=%new.path%& set JAVA_HOME=%java.home%& set display.jvm.list=%display.jvm.list%& set jvm.root=%jvm.root%& set mode.silent=%mode.silent%
...
:look.for.jvm.root
if not exist %jvm.root%\NUL goto found.jvm.root
...
for /d %%d in (%jvm.root%\j*) do (
	...
	if exist %%d\bin\NUL (
		...
		echo Attention ^^! Le répertoire !java.home.bin.current! ne contient pas de répertoire bin
...
if not defined jvm.root.missing goto verify.jvm.root.shell
goto error.in.execution.context

:verify.jvm.root.shell
call :look.for.jvm.root.shell
if not defined jvm.root.shell.missing goto verify.jvm.root.installations
goto error.in.execution.context

:verify.jvm.root.installations
call :check.for.jvm.root.installations
if %jvm.count% NEQ 0 goto verify.jvm.root.bin.installations
goto error.in.execution.context
...
:is.upper.alpha
set "not.alpha="&for /f "delims=ABCDEFGHIJKLMNOPQRSTUVWXYZ" %%i in ("!1!") do set not.alpha=%%i
goto :eof
